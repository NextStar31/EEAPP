...
export default class EventsScreen extends React.Component {
    ...
    fetch("http://192.168.1.26:8080/api/events", {
      ...
      .then((response) => {
        if (response.status == "success") {
          ...
          this.setState({
            events: response.data.sort((a, b) => {
              const aSplit = a.date.split("/");
              const bSplit = b.date.split("/");
              const dateA = new Date(aSplit[2], aSplit[1], aSplit[0]);
              const dateB = new Date(bSplit[2], bSplit[1], bSplit[0]);
              console.log(dateA)
              console.log(dateB)
  ...
  _addOneItem = () => {
    ...
    const item = {
      ...
      eventDay: "Dimanche",
      uri:
        "https://www.personalizationmall.com/cat_image/2000/18042-41306-200204155706.jpg",
      speakers: [
        {
      ...
      <SafeAreaView style={styles.container}>
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity
              onPress={() => {
                navigate("EventDetails", { id: item.id, name: item.title });
              }}
            >
